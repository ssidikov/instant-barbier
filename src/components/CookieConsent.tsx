    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className='fixed bottom-6 right-6 z-999 max-w-sm w-[calc(100vw-3rem)]'>
          <div className='relative bg-navy/95 backdrop-blur-xl border border-gold/15 shadow-2xl shadow-black/40 p-6'>
            ...
            <div className='flex items-center gap-3'>
              <button
                onClick={accept}
                className='group relative flex-1 py-2.5 text-[10px] uppercase tracking-[0.2em] font-semibold transition-all duration-500 overflow-hidden'>
                <span className='absolute inset-0 bg-gold' />
                <span className='absolute inset-0 bg-gold/80 scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500' />
                <span className='relative text-navy'>Accepter</span>
              </button>
              <button
                onClick={decline}
                className='group py-2.5 px-4 text-[10px] uppercase tracking-[0.2em] text-cream/40 hover:text-cream/70 border border-gold/10 hover:border-gold/30 transition-all duration-500'>
                Refuser
              </button>
            </div>
          </div>
